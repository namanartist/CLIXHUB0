...
const Footer: React.FC<Props> = ({ onOpenDeveloper, onOpenDeveloperProfile, onOpenProfile, onNavigate, isDarkMode, variant = 'default' }) => {
    const socialLinks = [
    ...
    return (
        <footer className="border-t border-[var(--border-color)] uni-glass-strong text-[var(--text-secondary)] transition-colors duration-500">
            <div className="max-w-[1600px] mx-auto px-6 md:px-8 py-8 md:py-12">

                {variant === 'default' && (
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-12 border-b border-[var(--border-color)] pb-12">

                        {/* Brand & Social - Spans 5 columns */}
                        <div className="col-span-1 md:col-span-5 space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl uni-btn-primary flex items-center justify-center text-white shadow-lg">
                                    <Zap size={20} className="fill-white" />
                                </div>
                                <div>
                                    <h3 className="text-xl font-black tracking-tight text-[var(--text-main)]">Clix Hub</h3>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">Club Lifecycle & Experience Hub</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {socialLinks.map((social, i) => (
                                    <a
                                        ...
                                        className="p-3 rounded-full bg-[var(--primary-soft)] hover:bg-[var(--primary-soft)] hover:text-[var(--text-main)] transition-all border border-transparent hover:border-[var(--border-color)]"
                                        title={social.label}
                                    >
                                        <social.icon size={18} />
                                    </a>
                                ...
                                <div className="w-px h-8 bg-[var(--primary-soft)] mx-2 hidden sm:block"></div>

                                <a
                                    href="http://moodle.mitsweb.in/"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="p-3 rounded-full bg-[#0099FF]/10 text-[#0099FF] hover:bg-[#0099FF] hover:text-[var(--text-main)] transition-all border border-[#0099FF]/20"
                                    title="Moodle (New)"
                                >
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="p-3 rounded-full bg-[var(--primary-soft)] hover:bg-[var(--primary-soft)] hover:text-[var(--text-main)] transition-all opacity-60 hover:opacity-100"
                                    title="Moodle (Legacy)"
                                >
                                    <GraduationCap size={18} />
                                </a>
                ...
                <div className="flex flex-col md:flex-row justify-center md:justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-1 rounded-full border transition-all ${isDarkMode
                        ...
                        <div className="flex items-center gap-2 text-xs font-bold">
                            ...
                            <button onClick={onOpenDeveloperProfile || onOpenProfile} className={`flex items-center gap-2 px-3 py-1 rounded-full ${isDarkMode ? 'bg-white/5 hover:bg-white/10 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-900'} transition-all`}>
